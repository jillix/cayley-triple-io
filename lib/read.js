...
ReadStream.prototype._read = function (size) {
    ...
    if (!self._dataAvailable) {
        ...
        _computeStartNodes(graph, self._query, self._prefixes, function (err, startNodes) {
            ...
            _getAllTriples(graph, startNodes, self._options.deep, self._options.out, self._options.projections, self._prefixes, function (err, triples) {
...
function _getAllTriples (graph, startNodes, deep, out, projections, prefixes, callback) {

    // expand current projections
    var cProjections = [];
    projections.forEach(function (projection) {
        if (projection instanceof Array) {
            cProjections.push(utils.parseValue(projection[0], prefixes));
        } else {
            cProjections.push(utils.parseValue(projection, prefixes));
        }
    });


    graph.V(startNodes).Tag('subject').Out(cProjections, 'predicate').Tag('object').All(function (err, triples) {
        ...
        async.each(triples, function (triple, cb) {
            ...
            if (utils.N3Util.isBlank(triple.object) || out.indexOf(triple.predicate) >= 0) {
                ...
                // build the next projections
                var nextProjections = [];
                if (cProjections.indexOf(triple.predicate) >= 0) {
                    if (projections[cProjections.indexOf(triple.predicate)] instanceof Array) {
                        nextProjections = projections[cProjections.indexOf(triple.predicate)][1];
                    }
                }

                _getAllTriples(graph, triple.object, true, out, nextProjections, prefixes, function (err, result) {
