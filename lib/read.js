...
function ReadStream (query, options) {
    ...
    for (var i = 0; i < this._options.out.length; ++i) {
        this._options.out[i] = utils.parseValue(this._options.out[i], prefixes);
    }
}
...
function _getAllTriples (graph, startNodes, deep, out, prefixes, callback) {
    ...
    graph.V(startNodes).Tag('subject').Out(null, 'predicate').Tag('object').All(function (err, triples) {
        ...
        async.each(triples, function (triple, cb) {
            ...
            if (utils.N3Util.isBlank(triple.object) || out.indexOf(triple.predicate) >= 0) {
                ...
                _getAllTriples(graph, triple.object, true, out, prefixes, function (err, result) {
